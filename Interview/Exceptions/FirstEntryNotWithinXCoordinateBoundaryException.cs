...
{
    /// <summary>FirstEntryNotWithinXCoordinateBoundaryException class</summary>
    public class FirstEntryNotWithinXCoordinateBoundaryException : Exception
    {
        /// <summary>Initializes a new instance of the FirstEntryNotWithinXCoordinateBoundaryException class</summary>
        /// <param name="roverCount">The rover number that is currently being reported in the input prompt</param>
        public FirstEntryNotWithinXCoordinateBoundaryException(int roverCount)
        {
            Console.Write("Rover {0}'s X coordinate is not within the graph's max X coordinate range.  Enter Rover {0} Starting Position (type Exit to quit anytime): ", roverCount);
        }
    }
}